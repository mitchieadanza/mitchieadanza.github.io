...
;( function() {
    ...
    TreeNode.prototype = {
        ...
        reset: function( nodeStructure, id, parentId, tree, stackParentId ) {
            ...
            this.hidden = nodeStructure.hidden || null;
            this.text = nodeStructure.text;
            this.class = nodeStructure.class || null;
    ...
    TreeNode.prototype.buildNodeFromText = function (node) {
        ...
        if (this.image) {
            ...
            node.className += " node-has-image";
        } else {
            node.className += " node-no-image";
        }

        // object in text
        var appendListToTextElement = function (list, element) {
            for (var innerKey in list) {
                if (!parseInt(innerKey)) {
                    node.className += " has-" + innerKey;
                }

                var textPartElement = document.createElement('span');
                if (!(list.length > 0)) {
                    textPartElement.className = innerKey;
                } else {
                    textPartElement.className = element.className + '-element';
                }

                if (typeof(list[innerKey]) == "object") {
                    appendListToTextElement(list[innerKey], textPartElement);
                } else {
                    textPartElement.appendChild(
                        document.createTextNode(list[innerKey])
                    );
                }
        ...
        if (this.text) {
            for (var key in this.text) {
                ...
                if (key.startsWith("data-")) {
                    ...
                    if (!parseInt(key)) {
                        node.className += " has-" + key;
                    }

                    // array within a key
                    if (typeof(this.text[key]) == "object") {
                        appendListToTextElement(this.text[key], textElement, this.hidden);
                    } else {
